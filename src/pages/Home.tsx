  ...
  return (
    <div>
      ...
      <section className="panel overflow-hidden p-8 sm:p-12">
        <p className="mb-4 inline-flex items-center gap-2 rounded-full border border-moss-400/40 bg-moss-400/10 px-3 py-1 text-xs font-medium text-moss-600">
          <span className="h-1.5 w-1.5 rounded-full bg-moss-400" />
          Runs entirely on your device
        </p>
        <h1 className="font-display text-3xl font-bold leading-tight tracking-tight text-ink-900 sm:text-5xl">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {TOOLS.map((tool) => (
            <Link
              key={tool.path}
              to={tool.path}
              className="group panel flex items-start justify-between gap-4 p-5 transition hover:border-rust-500/60 hover:bg-paper-200"
            >
              <div>
                <h3 className="font-display text-lg font-bold text-ink-900">{tool.heading}</h3>
                <p className="mt-1 text-sm text-ink-700">{tool.card}</p>
              </div>
              <span className="shrink-0 rounded-full border border-paper-400 px-2.5 py-1 text-[11px] text-ink-700 group-hover:border-rust-500/60 group-hover:text-rust-600">
                {tool.tag}
              </span>
            </Link>
          ))}
      ...
      <section className="mt-12 grid gap-4 sm:grid-cols-3">
        {HIGHLIGHTS.map((item) => (
          <div key={item.heading} className="panel p-5">
            <h2 className="font-display text-base font-bold text-ink-900">{item.heading}</h2>
      ...
      <section className="mt-12 max-w-3xl">
        ...
        <p className="mt-3 text-sm leading-relaxed text-ink-700">
          ...
          <Link to="/about" className="text-rust-600 underline underline-offset-4">
            how it works
          </Link>
          .
        </p>
        ...
        <p className="mt-4 text-sm text-ink-700">
          More answers on the{' '}
          <Link to="/faq" className="text-rust-600 underline underline-offset-4">
            FAQ page
          </Link>
          .
        </p>
